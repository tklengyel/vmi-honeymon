...
void honeymon_log_meminfo(honeymon_t *honeymon, uint32_t logIDX,
        ...
        uint64_t max_mem) {
    if (honeymon->log->mysql_enabled)
        honeymon_mysql_log_meminfo(honeymon, logIDX, current_mem, shared_mem,
                paged_mem, max_mem);
}
...
void honeymon_log_membenchmark(honeymon_t *honeymon, uint32_t logIDX,
    ...
    if (honeymon->log->mysql_enabled)
        honeymon_mysql_log_membenchmark(honeymon, logIDX, current_mem,
                shared_mem, max_mem);
}
